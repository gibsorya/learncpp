...
// macOS specific hints
  ...
  */
  ...
  popCat.texUnit(shaderProgram, "tex0", 0);
    ...
    // enables depth testing. we need this in order for OpenGL to know to check for depth in a 3D space
    glEnable(GL_DEPTH_TEST);
  // Main while loop
  while (!glfwWindowShouldClose(window)) {
    // Specify the color of the background
    glClearColor(0.07f, 0.13f, 0.17f, 1.0f);
    // Clean the back buffer and assign the new color to it. also clear the depth buffer
